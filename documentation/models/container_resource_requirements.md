...
| StorageAmount | int64    | ❌       | The amount of storage (in bytes) required by the container. Must be between 1 GB (1073741824 bytes) and 250 GB (268435456000 bytes). |
| ShmSize       | int64    | ❌       | The size of the shared memory (/dev/shm) in MB. If not specified, defaults to 1024MB.                                                |
